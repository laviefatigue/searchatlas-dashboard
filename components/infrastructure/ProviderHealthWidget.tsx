...
export function ProviderHealthWidget({ providers }: ProviderHealthProps) {
  const getProviderIcon = (name: string) => {
    if (name.toLowerCase().includes('microsoft') || name.toLowerCase().includes('entra')) {
      return <Cloud className="h-5 w-5 text-[#1DA89D]" />;
    }
    if (name.toLowerCase().includes('google') || name.toLowerCase().includes('gmail')) {
      return <Mail className="h-5 w-5 text-[#2C64D2]" />;
    }
  ...
  const getHealthColor = (score: number) => {
    ...
    if (score >= 60) return 'text-[#1DA89D]';
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-gray-300 transition-colors">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-2">
          <Server className="h-4 w-4 text-[#1DA89D]" />
          Provider Health
        </h3>
      </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#6E7C78]">
                    {provider.name}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-medium text-[#6E7C78] ml-auto">{provider.live_count}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-red-500" />
                  <span className="text-gray-500">Dead</span>
                  <span className="font-medium text-[#6E7C78] ml-auto">{provider.dead_count}</span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${barWidth}%`,
                    background: `linear-gradient(90deg, #1DA89D, #5ed0fd)`,
                  }}
                />
              </div>
            </div>
