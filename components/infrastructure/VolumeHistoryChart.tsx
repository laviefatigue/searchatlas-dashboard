'use client';

import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import type { VolumeHistory } from '@/lib/infrastructure-types';

interface VolumeHistoryChartProps {
  data: VolumeHistory;
}

export function VolumeHistoryChart({ data }: VolumeHistoryChartProps) {
  const chartData = data.snapshots.map((snapshot) => ({
    date: new Date(snapshot.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    'Emails Sent': snapshot.emails_sent,
    'Capacity': snapshot.daily_capacity_available,
  }));

  return (
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="gradientSent" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#388FC0" stopOpacity={0.4} />
              <stop offset="100%" stopColor="#388FC0" stopOpacity={0.02} />
            </linearGradient>
            <linearGradient id="gradientCapacity" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#6B8BA0" stopOpacity={0.4} />
              <stop offset="100%" stopColor="#6B8BA0" stopOpacity={0.02} />
            </linearGradient>
          </defs>
          <CartesianGrid
            strokeDasharray="3 3"
            stroke="#E7EAEC"
            vertical={false}
          />
          <XAxis
            dataKey="date"
            tick={{ fontSize: 11, fill: '#7A8892' }}
            stroke="#E7EAEC"
            tickLine={false}
            axisLine={false}
          />
          <YAxis
            tick={{ fontSize: 11, fill: '#7A8892' }}
            stroke="#E7EAEC"
            tickLine={false}
            axisLine={false}
            tickFormatter={(value) => value >= 1000 ? `${(value / 1000).toFixed(0)}k` : value}
          />
          <Tooltip
            contentStyle={{
              backgroundColor: '#ffffff',
              border: '1px solid #E7EAEC',
              borderRadius: '12px',
              boxShadow: '0 4px 16px rgba(0, 0, 0, 0.1)',
            }}
            labelStyle={{ color: '#6B7A85', fontWeight: 500, marginBottom: 4 }}
            itemStyle={{ color: '#388FC0' }}
          />
          <Legend
            wrapperStyle={{ fontSize: '12px', paddingTop: '12px' }}
            iconType="circle"
          />
          <Area
            type="monotone"
            dataKey="Emails Sent"
            stroke="#388FC0"
            strokeWidth={2}
            fillOpacity={1}
            fill="url(#gradientSent)"
          />
          <Area
            type="monotone"
            dataKey="Capacity"
            stroke="#6B8BA0"
            strokeWidth={2}
            fillOpacity={1}
            fill="url(#gradientCapacity)"
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
}
