...
function SentimentBadge({ sentiment }: { sentiment: ReplySentiment }) {
  const config = {
    positive: { label: 'Positive', bg: 'bg-guardare-lime/20', text: 'text-guardare-lime', icon: ThumbsUp },
...
function IntentBadge({ intent }: { intent: string }) {
  const colors: Record<string, string> = {
    'interested': 'bg-guardare-lime/20 text-guardare-lime',
    'not-interested': 'bg-red-950/30 text-red-400',
    'needs-info': 'bg-guardare-lime/20 text-guardare-lime',
    'referral': 'bg-guardare-purple/10 text-guardare-purple',
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-3">
              {showInterested && item.interestedCount > 0 && (
                <span className="text-xs text-guardare-lime font-medium">
...
function SentimentOverview({
  ...
  const items = [
    { key: 'positive' as const, label: 'Positive', color: 'bg-guardare-lime', count: breakdown.positive },
...
function ConversionFunnel({ funnel }: { funnel: FastAnalytics['funnel'] }) {
  const steps = [
    ...
    { label: 'Contacted', value: funnel.contacted, color: 'from-guardare-lime/70 to-guardare-lime/50' },
    { label: 'Replied', value: funnel.replied, color: 'from-guardare-purple/70 to-guardare-purple/50' },
    { label: 'Interested', value: funnel.interested, color: 'from-guardare-lime/70 to-guardare-lime/50' },
  ];
...
function CampaignComparison({ campaigns }: { campaigns: CampaignComparisonItem[] }) {
  ...
  return (
    <div className="rounded-lg border bg-card shadow-sm overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-guardare-lime/10 to-guardare-dark/10 border-b">
            ...
            {campaigns.map((c) => (
              <React.Fragment key={c.id}>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full flex-shrink-0 ${c.status.toLowerCase() === 'active' ? 'bg-guardare-lime' : 'bg-gray-500'}`} />
                  ...
                  <td className="py-3 px-4 text-right">
                    <span className={`font-bold ${c.interestRate === bestInterest && c.interestRate > 0 ? 'text-guardare-lime' : ''}`}>
                      {c.interestRate}%
                    </span>
                    {c.interested > 0 && (
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center gap-2 justify-end">
                      <div className="w-16 h-1.5 bg-muted rounded-full overflow-hidden">
                        <div
                          className="h-full bg-guardare-lime rounded-full"
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="bg-card rounded-lg p-3 border">
                            ...
                            <p className="font-bold text-lg text-guardare-lime">{c.interested.toLocaleString()}</p>
                          </div>
                        </div>

                        {/* Email Sequence */}
                        <div className="bg-card rounded-lg border overflow-hidden">
                          <div className="bg-gradient-to-r from-guardare-lime/10 to-guardare-dark/10 px-4 py-3 border-b">
                            <div className="flex items-center gap-2">
                              <Mail className="h-4 w-4 text-guardare-lime" />
                              <p className="text-sm font-semibold text-guardare-lime">Email Sequence</p>
                            </div>
                          </div>
                          <div className="p-4">
                            {seqLoading === c.id ? (
                              <div className="flex items-center justify-center py-8">
                                <Loader2 className="h-6 w-6 animate-spin text-guardare-lime" />
                                ...
                                {sequenceData[c.id].map((step, idx) => (
                                  <div key={step.id || idx} className="border rounded-lg overflow-hidden">
                                    <div className="bg-muted/50 px-4 py-2 flex items-center justify-between">
                                      <div className="flex items-center gap-2">
                                        <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-guardare-lime/20 text-guardare-lime text-xs font-bold">
                                          {idx + 1}
                                        </span>
                                        <span className="text-sm font-medium">Step {idx + 1}</span>
                                        {step.is_variant && (
                                          <span className="text-xs bg-guardare-purple/10 text-guardare-lime px-2 py-0.5 rounded">
                                            Variant {step.variant_letter || ''}
                                          </span>
                                        )}
                                        {step.is_thread_reply && (
                                          <span className="text-xs bg-guardare-lime/20 text-guardare-lime px-2 py-0.5 rounded">
                                            Thread Reply
                                          </span>
                                        )}
                                      </div>
                                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                        ...
                                        {step.reply_rate !== undefined && <span className="font-medium text-guardare-lime">{step.reply_rate}% reply</span>}
                                      ...
                                      {step.body ? (
                                        ...
                                        <div className="flex items-center gap-4 text-sm bg-muted/30 rounded-lg p-3">
                                          ...
                                          {step.unique_replies !== undefined && (
                                            <div><span className="text-muted-foreground">Replies: </span><span className="font-medium text-guardare-lime">{step.unique_replies}</span></div>
                                          )}
                                          {step.interested !== undefined && (
                                            <div><span className="text-muted-foreground">Interested: </span><span className="font-medium text-guardare-lime">{step.interested}</span></div>
                                          )}
                        ...
                        {/* Performance Summary */}
                        <div className="bg-gradient-to-r from-guardare-lime/10 to-guardare-dark/10 rounded-lg p-4 border border-guardare-lime/10">
                          <div className="flex items-center gap-2 mb-2">
                            <Target className="h-4 w-4 text-guardare-lime" />
                            <p className="text-xs text-guardare-lime uppercase tracking-wider font-semibold">Performance Summary</p>
                          </div>
                          <p className="text-sm text-muted-foreground">
                            ...
                            {' '}&rarr; <span className="font-semibold text-guardare-lime">{c.interested.toLocaleString()}</span> interested ({c.interestRate}%)
                          </p>
                        </div>
                      </div>
                    </td>
  ...
  return (
    <div className="rounded-lg border bg-card shadow-sm p-6">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-4">
                ...
                {step.totalInterested > 0 && (
                  <span className="text-xs font-medium text-guardare-lime">{step.interestRate}% interest ({step.totalInterested})</span>
                )}
              </div>
            </div>
            <div className="h-3 bg-muted rounded-full overflow-hidden flex">
              <div
                className="h-full bg-guardare-lime rounded-l-full transition-all duration-500"
                style={{ width: `${(step.replyRate / maxReplyRate) * 100}%` }}
                title={`${step.replyRate}% reply rate`}
              />
              {step.interestRate > 0 && (
                <div
                  className="h-full bg-guardare-lime transition-all duration-500"
      ...
      <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-guardare-lime rounded" /> Reply rate</div>
        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-guardare-lime rounded" /> Interest rate</div>
      </div>
    </div>
  );
}
            ...
            {data.byProvider.map((p) => (
              <div key={p.provider} className="p-4 rounded-lg bg-muted/50 border">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div className="min-w-0">
                    <p className="text-base font-bold text-guardare-lime">{p.replied.toLocaleString()}</p>
          ...
          {activeDomains.length > 10 && (
            <div className="p-3 text-center border-t">
              <button
                ...
                className="text-xs text-guardare-lime hover:text-guardare-lime/80 font-medium"
              >
...
function PipelineCompanies({
  ...
  return (
    <div className="rounded-lg border bg-card shadow-sm p-6">
      <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
        <Building2 className="h-4 w-4 text-guardare-lime" />
        Pipeline Companies
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {companies.map((c) => {
          ...
          return (
            <div key={c.label} className="p-3 rounded-lg bg-guardare-lime/10 border border-guardare-lime/20">
              <div className="flex items-center justify-between mb-1">
                {domain ? (
                  <a href={`https://${domain}`} target="_blank" rel="noopener noreferrer" className="font-medium text-sm truncate max-w-[180px] text-guardare-lime hover:underline" title={c.label}>{c.label}</a>
                ) : (
                  <span className="font-medium text-sm truncate max-w-[180px]" title={c.label}>{c.label}</span>
                )}
                <span className="text-xs bg-guardare-lime/20 text-guardare-lime px-1.5 py-0.5 rounded font-medium">
              ...
              {hasEnrichment && (
                <div className="mt-2 pt-2 border-t border-guardare-lime/10 flex flex-wrap items-center gap-2">
                  {enrichData.linkedinUrl && (
                    <a
                      href={enrichData.linkedinUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[11px] text-guardare-lime hover:underline"
                    >
...
function LeadDeepDive({
  ...
  return (
    <div className="rounded-lg border bg-card shadow-sm overflow-hidden">
      <div className="p-4 bg-gradient-to-r from-guardare-lime/10 to-guardare-dark/10 border-b space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-guardare-purple/20 rounded-lg flex items-center justify-center">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                showFilters ? 'bg-guardare-purple text-white border-guardare-purple' : 'bg-secondary hover:bg-accent'
              }`}
            >
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-48 h-9 pl-9 pr-3 text-sm rounded-lg border bg-background focus:outline-none focus:ring-2 focus:ring-guardare-lime/20"
              />
            </div>
          </div>
        </div>
              ...
              <>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    ...
                    {reply.title && <p className="text-xs text-guardare-lime">{reply.title}</p>}
                  </td>
                  <td className="py-3 px-4">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {reply.buyingSignals.length > 0 && (
                            <div className="bg-guardare-lime/10 rounded-lg p-3 border border-guardare-lime/20">
                              <div className="flex items-center gap-1 mb-2">
                                <Sparkles className="h-3 w-3 text-guardare-lime" />
                                <p className="text-xs font-semibold text-guardare-lime uppercase">Buying Signals</p>
                              </div>
                              <ul className="space-y-1">{reply.buyingSignals.map((s, i) => <li key={i} className="text-xs text-guardare-lime">{s}</li>)}</ul>
                            </div>
                          )}
                          ...
                          {reply.themes.length > 0 && (
                            <div className="bg-guardare-lime/10 rounded-lg p-3 border border-guardare-lime/20">
                              <div className="flex items-center gap-1 mb-2">
                                <MessageSquare className="h-3 w-3 text-guardare-lime" />
                                <p className="text-xs font-semibold text-guardare-lime uppercase">Themes</p>
                              </div>
                              <div className="flex flex-wrap gap-1">{reply.themes.map((t, i) => <span key={i} className="text-xs bg-guardare-lime/20 text-guardare-lime px-2 py-0.5 rounded">{t}</span>)}</div>
                            </div>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
                          ...
                          {reply.isInterested && <span className="text-guardare-lime font-medium">Marked Interested</span>}
                        </div>
                      </div>
                    </td>
                  </tr>
    ...
    <PageContainer className="space-y-8 pb-12">
      ...
      <div className="flex items-center justify-end gap-3 hide-on-export">
        ...
        <button onClick={handleExportPDF} disabled={exporting}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-guardare-purple hover:bg-guardare-purple/80 text-white transition-colors disabled:opacity-50">
      ...
      <div ref={contentRef} className="space-y-8">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 hide-on-export">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg border transition-colors whitespace-nowrap ${
                activeCycle === null
                  ? 'bg-guardare-purple text-white border-guardare-purple'
        ...
        {hero && (
          <div className="bg-gradient-to-br from-guardare-purple/90 to-guardare-dark text-white rounded-3xl p-8 shadow-xl">
        ...
        {phase3Loading ? (
          ...
          <div className="rounded-lg border bg-card shadow-sm overflow-hidden">
            <div className="flex flex-col space-y-1.5 p-6 bg-gradient-to-r from-guardare-lime/10 to-guardare-dark/10 border-b">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-guardare-lime/10 p-4 rounded-xl border border-guardare-lime/20">
                  <div className="flex items-center gap-2 mb-2">
                    <MessageSquare className="h-4 w-4 text-guardare-lime" />
                    <h4 className="font-semibold text-guardare-lime text-sm">Subject Lines</h4>
                  </div>
                  <p className="text-sm text-guardare-lime/80">
                ...
                {copyData.body?.analysis && (
                  <div className="bg-guardare-lime/10 p-4 rounded-xl border border-guardare-lime/20">
                    <div className="flex items-center gap-2 mb-2">
                      <Target className="h-4 w-4 text-guardare-lime" />
                      <h4 className="font-semibold text-guardare-lime text-sm">Opening Hooks</h4>
                    </div>
                    <p className="text-sm text-guardare-lime/80">
                ...
                {copyData.cta?.analysis && (
                  <div className="bg-guardare-lime/10 p-4 rounded-xl border border-guardare-lime/20">
                    <div className="flex items-center gap-2 mb-2">
                      <Mail className="h-4 w-4 text-guardare-lime" />
                      <h4 className="font-semibold text-guardare-lime text-sm">CTAs</h4>
                    </div>
                    <p className="text-sm text-guardare-lime/80">
          ...
          <>
            ...
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-4 flex items-center gap-2">
                <Brain className="h-6 w-6 text-guardare-lime" />
                Response Intelligence
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="rounded-lg border bg-card shadow-sm p-6">
                ...
                {filteredReport.topBuyingSignals.length > 0 && (
                  <div className="rounded-lg border bg-card shadow-sm p-6">
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                      <Sparkles className="h-4 w-4 text-guardare-lime" /> Buying Signals
                    </h3>
                    <div className="space-y-2">
                      {filteredReport.topBuyingSignals.map((s) => (
                        <div key={s.signal} className="flex items-center justify-between">
                          <span className="text-sm font-medium text-guardare-lime">{toTitleCase(s.signal)}</span>
            ...
            <div>
              <h2 className="text-2xl font-bold text-foreground mb-4 flex items-center gap-2">
                <BarChart3 className="h-6 w-6 text-guardare-lime" />
                Who&apos;s Responding
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="rounded-lg border bg-card shadow-sm p-6">
                  ...
                  <HorizontalBarChart data={filteredReport.industryDistribution} colorClass="bg-guardare-lime" showInterested />
                </div>
                <div className="rounded-lg border bg-card shadow-sm p-6">
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                    <Briefcase className="h-4 w-4" /> Title Seniority
                  </h3>
                  <HorizontalBarChart data={filteredReport.seniorityDistribution} colorClass="bg-guardare-purple" showInterested />
                </div>
