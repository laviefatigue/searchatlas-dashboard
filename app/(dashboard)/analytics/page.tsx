...
function SentimentBadge({ sentiment }: { sentiment: ReplySentiment }) {
  const config = {
    positive: { label: 'Positive', bg: 'bg-brand-green/20', text: 'text-brand-green', icon: ThumbsUp },
...
function IntentBadge({ intent }: { intent: string }) {
  const colors: Record<string, string> = {
    'interested': 'bg-brand-green/20 text-brand-green',
    'not-interested': 'bg-red-950/30 text-red-400',
    'needs-info': 'bg-brand-cyan/20 text-brand-cyan',
...
function SentimentOverview({
  ...
  const items = [
    { key: 'positive' as const, label: 'Positive', color: 'bg-brand-green', count: breakdown.positive },
...
function ConversionFunnel({ funnel }: { funnel: FastAnalytics['funnel'] }) {
  const steps = [
    ...
    { label: 'Contacted', value: funnel.contacted, color: 'from-brand-cyan/70 to-brand-cyan/50' },
            ...
            {campaigns.map((c) => (
              <React.Fragment key={c.id}>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full flex-shrink-0 ${c.status.toLowerCase() === 'active' ? 'bg-brand-green' : 'bg-gray-500'}`} />
                  ...
                  <td className="py-3 px-4 text-right">
                    <span className={`font-bold ${c.interestRate === bestInterest && c.interestRate > 0 ? 'text-brand-green' : ''}`}>
                      {c.interestRate}%
                    </span>
                    {c.interested > 0 && (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="bg-card rounded-lg p-3 border">
                            ...
                            <p className="font-bold text-lg text-brand-green">{c.interested.toLocaleString()}</p>
                          </div>
                        </div>

                        {/* Email Sequence */}
                        <div className="bg-card rounded-lg border overflow-hidden">
                          <div className="bg-gradient-to-r from-brand-cyan/10 to-brand-dark/10 px-4 py-3 border-b">
                            <div className="flex items-center gap-2">
                              <Mail className="h-4 w-4 text-brand-cyan" />
                              <p className="text-sm font-semibold text-brand-cyan">Email Sequence</p>
                            </div>
                          </div>
                          <div className="p-4">
                            {seqLoading === c.id ? (
                                ...
                                {sequenceData[c.id].map((step, idx) => (
                                  <div key={step.id || idx} className="border rounded-lg overflow-hidden">
                                    <div className="bg-muted/50 px-4 py-2 flex items-center justify-between">
                                      <div className="flex items-center gap-2">
                                        <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-brand-cyan/20 text-brand-cyan text-xs font-bold">
                                          {idx + 1}
                                        </span>
                                        <span className="text-sm font-medium">Step {idx + 1}</span>
                                        {step.is_variant && (
                                          <span className="text-xs bg-brand-purple/10 text-brand-cyan px-2 py-0.5 rounded">
                                      ...
                                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                        ...
                                        {step.reply_rate !== undefined && <span className="font-medium text-brand-cyan">{step.reply_rate}% reply</span>}
                                      ...
                                      {step.body ? (
                                        ...
                                        <div className="flex items-center gap-4 text-sm bg-muted/30 rounded-lg p-3">
                                          ...
                                          {step.unique_replies !== undefined && (
                                            <div><span className="text-muted-foreground">Replies: </span><span className="font-medium text-brand-cyan">{step.unique_replies}</span></div>
                                          )}
                                          {step.interested !== undefined && (
                                            <div><span className="text-muted-foreground">Interested: </span><span className="font-medium text-brand-green">{step.interested}</span></div>
                                          )}
                        ...
                        {/* Performance Summary */}
                        <div className="bg-gradient-to-r from-brand-cyan/10 to-brand-dark/10 rounded-lg p-4 border border-brand-cyan/10">
                          <div className="flex items-center gap-2 mb-2">
                            <Target className="h-4 w-4 text-brand-cyan" />
                            <p className="text-xs text-brand-cyan uppercase tracking-wider font-semibold">Performance Summary</p>
                          </div>
                          <p className="text-sm text-muted-foreground">
                            ...
                            {' '}&rarr; <span className="font-semibold text-brand-green">{c.interested.toLocaleString()}</span> interested ({c.interestRate}%)
                          </p>
                        </div>
                      </div>
                    </td>
  ...
  return (
    <div className="rounded-lg border bg-card shadow-sm p-6">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-4">
                ...
                {step.totalInterested > 0 && (
                  <span className="text-xs font-medium text-brand-green">{step.interestRate}% interest ({step.totalInterested})</span>
                )}
              </div>
            </div>
            <div className="h-3 bg-muted rounded-full overflow-hidden flex">
              <div
                className="h-full bg-brand-cyan rounded-l-full transition-all duration-500"
      ...
      <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-brand-cyan rounded" /> Reply rate</div>
        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-brand-green rounded" /> Interest rate</div>
      </div>
    </div>
  );
}
            ...
            {data.byProvider.map((p) => (
              <div key={p.provider} className="p-4 rounded-lg bg-muted/50 border">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div className="min-w-0">
                    <p className="text-base font-bold text-brand-cyan">{p.replied.toLocaleString()}</p>
    ...
    <div className="rounded-lg border bg-card shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {companies.map((c) => {
          ...
          return (
            <div key={c.label} className="p-3 rounded-lg bg-brand-green/10 border border-brand-green/20">
              <div className="flex items-center justify-between mb-1">
                {domain ? (
                  <a href={`https://${domain}`} target="_blank" rel="noopener noreferrer" className="font-medium text-sm truncate max-w-[180px] text-brand-cyan hover:underline" title={c.label}>{c.label}</a>
                ) : (
                  <span className="font-medium text-sm truncate max-w-[180px]" title={c.label}>{c.label}</span>
                )}
                <span className="text-xs bg-brand-green/20 text-brand-green px-1.5 py-0.5 rounded font-medium">
              ...
              {hasEnrichment && (
                <div className="mt-2 pt-2 border-t border-brand-green/10 flex flex-wrap items-center gap-2">
              ...
              <>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    ...
                    {reply.title && <p className="text-xs text-brand-cyan">{reply.title}</p>}
                  </td>
                  <td className="py-3 px-4">
                    ...
                    <td colSpan={7} className="p-6">
                      <div className="space-y-4">
                          ...
                          {reply.buyingSignals.length > 0 && (
                            <div className="bg-brand-green/10 rounded-lg p-3 border border-brand-green/20">
                              <div className="flex items-center gap-1 mb-2">
                                ...
                                <p className="text-xs font-semibold text-brand-green uppercase">Buying Signals</p>
                              </div>
                              <ul className="space-y-1">{reply.buyingSignals.map((s, i) => <li key={i} className="text-xs text-brand-green">{s}</li>)}</ul>
                            </div>
                          )}
                          ...
                          {reply.themes.length > 0 && (
                            <div className="bg-brand-cyan/10 rounded-lg p-3 border border-brand-cyan/20">
                              <div className="flex items-center gap-1 mb-2">
                                ...
                                <p className="text-xs font-semibold text-brand-cyan uppercase">Themes</p>
                              </div>
                              <div className="flex flex-wrap gap-1">{reply.themes.map((t, i) => <span key={i} className="text-xs bg-brand-cyan/20 text-brand-cyan px-2 py-0.5 rounded">{t}</span>)}</div>
                            </div>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
                          ...
                          {reply.isInterested && <span className="text-brand-green font-medium">Marked Interested</span>}
                        </div>
                      </div>
                    </td>
                  </tr>
    ...
    <PageContainer className="space-y-8 pb-12">
      ...
      <div className="flex items-center justify-end gap-3 hide-on-export">
        ...
        <button onClick={handleExportPDF} disabled={exporting}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-brand-purple hover:bg-brand-purple/80 text-white transition-colors disabled:opacity-50">
      ...
      <div ref={contentRef} className="space-y-8">
        ...
        {hero && (
          <div className="bg-gradient-to-br from-brand-purple/90 to-brand-dark text-white rounded-3xl p-8 shadow-xl">
        ...
        {phase3Loading ? (
          ...
          <div className="rounded-lg border bg-card shadow-sm overflow-hidden">
            <div className="flex flex-col space-y-1.5 p-6 bg-gradient-to-r from-brand-cyan/10 to-brand-dark/10 border-b">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-brand-cyan/10 p-4 rounded-xl border border-brand-cyan/20">
                  <div className="flex items-center gap-2 mb-2">
                    <MessageSquare className="h-4 w-4 text-brand-cyan" />
                    <h4 className="font-semibold text-brand-cyan text-sm">Subject Lines</h4>
                  </div>
                ...
                {copyData.body?.analysis && (
                  <div className="bg-brand-cyan/10 p-4 rounded-xl border border-brand-cyan/20">
                    <div className="flex items-center gap-2 mb-2">
                      <Target className="h-4 w-4 text-brand-cyan" />
                      <h4 className="font-semibold text-brand-cyan text-sm">Opening Hooks</h4>
                    </div>
                    <p className="text-sm text-brand-cyan/80">
                ...
                {copyData.cta?.analysis && (
                  <div className="bg-brand-cyan/10 p-4 rounded-xl border border-brand-cyan/20">
                    <div className="flex items-center gap-2 mb-2">
                      <Mail className="h-4 w-4 text-brand-cyan" />
                      <h4 className="font-semibold text-brand-cyan text-sm">CTAs</h4>
                    </div>
                    <p className="text-sm text-brand-cyan/80">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {filteredReport.topBuyingSignals.length > 0 && (
                  <div className="rounded-lg border bg-card shadow-sm p-6">
                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                      <Sparkles className="h-4 w-4 text-brand-green" /> Buying Signals
                    </h3>
                    <div className="space-y-2">
                      {filteredReport.topBuyingSignals.map((s) => (
                        <div key={s.signal} className="flex items-center justify-between">
                          <span className="text-sm font-medium text-brand-green">{toTitleCase(s.signal)}</span>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="rounded-lg border bg-card shadow-sm p-6">
                  ...
                  <HorizontalBarChart data={filteredReport.industryDistribution} colorClass="bg-brand-cyan" showInterested />
                </div>
                <div className="rounded-lg border bg-card shadow-sm p-6">
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                    <Briefcase className="h-4 w-4" /> Title Seniority
                  </h3>
                  <HorizontalBarChart data={filteredReport.seniorityDistribution} colorClass="bg-brand-purple" showInterested />
                </div>
